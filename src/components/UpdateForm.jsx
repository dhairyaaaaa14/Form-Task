...
const UpdateForm = () => {
  ...
  const updateData = async (row) => {
    const response = await api.put(`/data/${row.id}`, row);
    // const {id,inputText,inputSelect,inputCheckbox,inputRadio} = response.data;
    // setFormData(
    //   formData.map((data) =>{
...
{/* 
          <div class="form-check form-check-inline">
            <Input
              ...
              type="checkbox"
              name="java"
              checked={formData.inputCheckbox.includes()}
              onChange={handleChange}
              value="java"
            />
            <label class="form-check-label" for="inlineCheckbox1">
          ...
          <br />
          <button className="btn btn-primary mx-5 col-4"  onClick={()=> updateData(formData)}>
            Update
          </button>
        </div>
      </form>
